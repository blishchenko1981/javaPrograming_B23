...
public class Catalog {
    ...
    public String getWholeCatalog() {
      ...
      for (int i = 0; i < items.size()-1; i++) {
               wholeCatalog +=  ""+ items.get(i) + "-" + prices.get(i) + "-" + monthlyPayments.get(i) + "\n";}
    ...
    public ArrayList<String> getItemsLessThanAMonthlyPrice(double price) {
        //LOAD WHOLE CATALOG HERE FIRST
           loadWholeCatalog();
        //TODO
        ArrayList<String> itemsLessThenMonPrice = new ArrayList<>();

        for (int i = 0; i < monthlyPayments.size()-1; i++) {
            if(monthlyPayments.get(i)<= price){
                itemsLessThenMonPrice.add(items.get(i));
            }
        }
        return itemsLessThenMonPrice;
    }
